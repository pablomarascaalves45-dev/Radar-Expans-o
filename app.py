...
# 2. Estilização personalizada (Mantida conforme original)
st.markdown("""
    <style>
...
def exportar_pdf(dados_cidade, endereco, lat_lon, obs, avaliacoes, concorrencia, polos, caracteristicas, foto_arquivo, score_final):
    pdf = FPDF()
    pdf.add_page()
    pdf.set_margins(10, 10, 10) # Margens estreitas
    
    # Título e Score
    pdf.set_font("Arial", "B", 14)
    pdf.cell(0, 8, txt="Relatorio de Expansao - Analise de Ponto", ln=True, align='C')
    pdf.set_text_color(0, 51, 102) # Azul escuro para o Score
    ...
    pdf.cell(0, 6, txt="1. Mercado da Cidade", ln=True, fill=False)
    ...
    # 2. Dados do Ponto e Foto (Lado a Lado ou Compacto)
    pdf.set_font("Arial", "B", 10)
    ...
    if foto_arquivo:
        try:
            ...
            img.save(img_path)
            # Imagem menor para economizar espaço
    ...
    # 3. Analise de Campo (Exibição em colunas para economizar espaço)
    pdf.set_font("Arial", "B", 10)
...
# =========================================================
# O restante da sua interface permanece idêntico
# =========================================================
...
if df is not None:
    ...
    with col_cp4: char_acess = st.selectbox("Acessibilidade", options=["Baixa", "Média", "Alta"])
    with col_cp5: char_vagas = st.selectbox("Vagas", options=["Não", "Sim"])
    ...
    with col_p1: polo_super = st.checkbox("Supermercado")
    with col_p2: polo_pada = st.checkbox("Padaria")
    with col_p3: polo_hosp = st.checkbox("Hospital/UPA")
    col_p4, col_p5, col_p6 = st.columns(3)
    with col_p4: polo_banc = st.checkbox("Bancos/Lotéricas")
    with col_p5: polo_pet = st.checkbox("PetShop")
    with col_p6: polo_fem = st.checkbox("Lojas público feminino")
    ...
    dados_polos = {"Supermercado": "Sim" if polo_super else "Não", "Padaria": "Sim" if polo_pada else "Não", "Hospital": "Sim" if polo_hosp else "Não", "Bancos": "Sim" if polo_banc else "Não", "PetShop": "Sim" if polo_pet else "Não", "Fem": "Sim" if polo_fem else "Não"}
    dados_caract = {"Local": char_local, "Posicao": char_posicao, "Visib.": char_visib, "Acess.": char_acess, "Vagas": char_vagas, "Sol": char_solar}

    st.markdown(f'<div class="score-container"><h1 style="color:white;">{score_final} pts</h1></div>', unsafe_allow_html=True)

    if st.button("🚀 Gerar Relatório de 1 Página"):
