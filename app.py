...
if not st.session_state.logado:
    ...
    def exportar_pdf(dados_cidade, endereco, obs, avaliacoes, concorrencia, polos, caracteristicas, foto_arquivo, perc_final, score_mercado, score_ponto, p_merc_txt, p_ponto_txt):
        ...
        col_w = 63
        pdf.cell(col_w, 5, txt=clean(f"Populacao: {formatar_br(dados_cidade.get('População', 0), 0)}"), ln=0)
        pdf.cell(col_w, 5, txt=clean(f"Lojas Atuais: {formatar_br(dados_cidade.get('N° FSJ', 0), 0)}"), ln=0)
        pdf.cell(col_w, 5, txt=clean(f"Renda Media: {formatar_br(dados_cidade.get('Renda Média Domiciliar (SM)', 0), 2)}"), ln=1)
        pdf.cell(col_w, 5, txt=clean(f"Share: {formatar_br(dados_cidade.get('%Share', 0) * 100, 2)}%"), ln=0)
        pdf.cell(col_w, 5, txt=clean(f"Demanda: {formatar_br(dados_cidade.get('Demanda', 0), 2)}"), ln=0)
        ...
        pdf.set_x(75)
        pdf.set_font("Arial", "B", 8)
        pdf.cell(60, 5, txt=clean("CARACTERISTICAS E POLOS"), ln=True)
        pdf.set_font("Arial", "", 8)
        ...
        pdf.set_x(140)
        pdf.set_font("Arial", "B", 8)
        pdf.cell(60, 5, txt=clean("OBSERVACOES DA VISTORIA:"), ln=True)
        pdf.set_font("Arial", "", 8)
        pdf.set_x(140)
        pdf.multi_cell(60, 4, txt=clean(obs))
        y_final_col3 = pdf.get_y()

        y_apos_tecnico = max(y_final_col1, y_final_col2, y_final_col3)
        pdf.set_y(y_apos_tecnico + 8)
        pdf.set_font("Arial", "B", 10)
        pdf.cell(0, 5, txt=clean("4. FOTO DO PONTO"), ln=True)
        pdf.ln(4) 
    ...
    if df is not None:
        ...
        if cidade_selecionada:
            ...
            if char_posicao == "Esquina +": 
                ...
                elif estado_cidade == "RS": score_ponto_calc -= 2
            elif char_posicao == "Meio de quadra > 20m":
